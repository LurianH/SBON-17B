...
export async function loadAnalysisData():Promise<AnalysisData>{
 ...
 const rows=specs.map(([label,key,suffix])=>{const current=latest[key]==null?null:Number(latest[key]),avg=base.some(r=>r[key]==null)?null:calculateProjectionBaseAverage(base.map(r=>Number(r[key]))),future=avg===null||current===null?null:calculateQuarterProjection(current,avg,[4.345,4.345]);return [label,current===null?'Aguardando atualização':fmt(current,suffix),future?fmt(future[0].accumulated,suffix):'Em formação',future?fmt(future[1].accumulated,suffix):'Em formação']});
