...
export async function loadDashboardData():Promise<DashboardData>{
 ...
 const [{data:targetRow},{data:financials},{data:financialSnapshots},{data:weekly},{count:openActions},engineering]=await Promise.all([
  ...
  db.from('financial_measurement_snapshots').select('reference_month,cumulative_amount').eq('contract_id',contract.id).eq('reference_year',year).order('reference_month',{ascending:false}).limit(1),
  db.from('weekly_progress').select('id,reference_date,economies_executed,network_executed_m').eq('contract_id',contract.id).eq('active',true).order('reference_date',{ascending:true}),
  db.from('action_plans').select('id',{count:'exact',head:true}).eq('contract_id',contract.id).neq('status','concluido'),
  loadDashboardEngineering(db,contract.id)
 ]);
 ...
 // The execution input is consolidated economies, so a missing target segment
 // cannot be isolated into a trustworthy segment-specific deviation.
 const compatibleSegments=!targetBreakdown||((targetBreakdown.water===0||engineering.engineeringBreakdown?.waterEconomies!=null)&&(targetBreakdown.sewer===0||engineering.engineeringBreakdown?.sewerEconomies!=null));
 const origin=target!==null&&latest&&engineering.available!==null&&compatibleSegments?calculateDeviationOrigin(target,elapsedYear(date),engineering.available,latest.economies_executed):null;
 return {source:'supabase',targetYear:year,contractValue:contract.contract_value===null?null:Number(contract.contract_value),billingYtd:financialSnapshots?.length?Number(financialSnapshots[0].cumulative_amount):financials?.length?calculateMonthlyBillingYTD(financials.map(r=>({year:r.reference_year,amount:Number(r.amount)})),year):null,target,targetBreakdown,...engineering,annualProjection:projection,latestDate:latest?.reference_date??null,executed:latest?.economies_executed??null,networkExecuted:latest?Number(latest.network_executed_m):null,deviation:projection!==null&&target!==null?calculateAnnualTargetDeviation(projection,target):null,status:projection!==null&&target!==null?calculateDashboardStatus(projection,target):null,origin:origin?{engineeringPercent:origin.engineeringPercent,executionPercent:origin.executionPercent,engineering:origin.engineering,execution:origin.execution}:null,openActions:openActions??0,weeklyExecuted:rows.slice(-7).map(r=>r.economies_executed),contractId:contract.id};
}
