...
export async function loadDashboardData():Promise<DashboardData>{
 ...
 const [{data:targetRow},{data:financials},{data:financialSnapshots},{data:weekly},{count:openActions}]=await Promise.all([
  ...
  db.from('weekly_progress').select('id,reference_date,economies_available,economies_executed,network_approved_m,network_executed_m').eq('contract_id',contract.id).eq('active',true).order('reference_date',{ascending:true}),
 ...
 const {data:progressDetails}=latest?await db.from('weekly_progress_details').select('segment_type,economies_available,network_approved_m').eq('weekly_progress_id',latest.id):{data:null};
 const engineeringBreakdown=progressDetails?.length?progressDetails.reduce((sum,row)=>{if(row.segment_type==='WATER'){sum.waterEconomies+=row.economies_available;sum.waterNetwork+=Number(row.network_approved_m)}else{sum.sewerEconomies+=row.economies_available;sum.sewerNetwork+=Number(row.network_approved_m)}return sum},{waterEconomies:0,sewerEconomies:0,waterNetwork:0,sewerNetwork:0}):null;
