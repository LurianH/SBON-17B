...
export interface DashboardData{source:'demo'|'supabase'|'empty';contractValue:number|null;billingYtd:number|null;target:number|null;annualProjection:number|null;latestDate:string|null;available:number|null;executed:number|null;networkApproved:number|null;networkExecuted:number|null;deviation:number|null;status:'DENTRO_DO_PLANO'|'ATENCAO'|'CRITICO'|null;origin:{engineeringPercent:number|null;executionPercent:number|null;engineering:number;execution:number}|null;openActions:number;weeklyExecuted:number[];contractId:string|null}
const empty:DashboardData={source:'empty',contractValue:null,billingYtd:null,target:null,annualProjection:null,latestDate:null,available:null,executed:null,networkApproved:null,networkExecuted:null,deviation:null,status:null,origin:null,openActions:0,weeklyExecuted:[],contractId:null};
const demo:DashboardData={source:'demo',contractValue:78500000,billingYtd:42300000,target:15000,annualProjection:14400,latestDate:'2026-08-24',available:13480,executed:11920,networkApproved:186400,networkExecuted:154300,deviation:-4,status:'ATENCAO',origin:{engineeringPercent:42,executionPercent:58,engineering:640,execution:880},openActions:3,weeklyExecuted:[10820,11040,11200,11480,11610,11770,11920],contractId:'demo'};
...
export async function loadDashboardData():Promise<DashboardData>{
 ...
 const [{data:targetRow},{data:financials},{data:weekly},{count:openActions}]=await Promise.all([
  db.from('annual_targets').select('target_economies').eq('contract_id',contract.id).eq('year',year).maybeSingle(),
 ...
 const rows=weekly??[],latest=rows.at(-1),target=targetRow?.target_economies??null,firstMonth=rows[0]?.reference_date.slice(0,7),baseRows=firstMonth?rows.filter(r=>r.reference_date.startsWith(firstMonth)):[];
